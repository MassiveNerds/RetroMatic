...
&lt;hr&gt;`;
  ...
  upVote() {
    ...
    if (this.activeNote.votes) {
      this.activeNote.votes[userId] = true;
    } else {
    ...
    this.db.object(`/notes/${this.activeBucket.$key}/${this.activeNote.$key}`).update({votes: this.activeNote.votes, totalVotes: this.activeNote.totalVotes})
